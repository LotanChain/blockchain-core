...
function _complete_besu() {
  ...
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} password" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} retesteth" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} rlp" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} validate-config" ];    then _picocli_besu; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks import" ];    then _picocli_besu_blocks; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} blocks export" ];    then _picocli_besu_blocks; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key export" ];    then _picocli_besu_publickey; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} public-key export-address" ];    then _picocli_besu_publickey; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} password hash" ];    then _picocli_besu_password; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} rlp encode" ];    then _picocli_besu_rlp; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator generate-blockchain-config" ];    then _picocli_besu_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator generate-log-bloom-cache" ];    then _picocli_besu_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator x-backup-state" ];    then _picocli_besu_operator; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} operator x-restore-state" ];    then _picocli_besu_operator; return $?; fi
...
function _picocli_besu() {
  ...
  local flag_opts="--p2p-enabled --discovery-enabled --remote-connections-limit-enabled --random-peer-priority-enabled --graphql-http-enabled --rpc-http-enabled --rpc-http-authentication-enabled --rpc-http-tls-enabled --rpc-http-tls-client-auth-enabled --rpc-http-tls-ca-clients-enabled --rpc-ws-enabled --engine-jwt-enabled --rpc-ws-authentication-enabled --privacy-tls-enabled --metrics-enabled --metrics-push-enabled --color-enabled --miner-enabled --miner-stratum-enabled --pruning-enabled --permissions-nodes-config-file-enabled --permissions-accounts-config-file-enabled --permissions-nodes-contract-enabled --permissions-accounts-contract-enabled --privacy-enabled --privacy-multi-tenancy-enabled --revert-reason-enabled --privacy-enable-database-migration --privacy-flexible-groups-enabled --auto-log-bloom-caching-enabled -h --help -V --version --compatibility-eth64-forkid-enabled --strict-tx-replay-protection-enabled"
  local arg_opts="--config-file --data-path --genesis-file --identity --bootnodes --max-peers --remote-connections-max-percentage --sync-mode --fast-sync-min-peers --network --p2p-host --p2p-interface --p2p-port --nat-method --network-id --graphql-http-host --graphql-http-port --graphql-http-cors-origins --rpc-http-host --rpc-http-port --engine-rpc-http-port --rpc-http-max-active-connections --rpc-http-cors-origins --rpc-http-api --rpc-http-apis --rpc-http-api-method-no-auth --rpc-http-api-methods-no-auth --rpc-http-authentication-credentials-file --rpc-http-authentication-jwt-public-key-file --rpc-http-authentication-jwt-algorithm --rpc-http-tls-keystore-file --rpc-http-tls-keystore-password-file --rpc-http-tls-known-clients-file --rpc-http-tls-protocol --rpc-http-tls-protocols --rpc-http-tls-cipher-suite --rpc-http-tls-cipher-suites --rpc-ws-authentication-jwt-algorithm --rpc-ws-host --rpc-ws-port --engine-rpc-ws-port --engine-jwt-secret --rpc-ws-max-frame-size --rpc-ws-max-active-connections --rpc-ws-api --rpc-ws-apis --rpc-ws-api-methods-no-auth --rpc-ws-api-method-no-auth --rpc-ws-authentication-credentials-file --rpc-ws-authentication-jwt-public-key-file --privacy-tls-keystore-file --privacy-tls-keystore-password-file --privacy-tls-known-enclave-file --metrics-protocol --metrics-host --metrics-port --metrics-category --metrics-categories --metrics-push-host --metrics-push-port --metrics-push-interval --metrics-push-prometheus-job --host-allowlist --engine-host-allowlist --reorg-logging-threshold --miner-stratum-host --miner-stratum-port --miner-coinbase --min-gas-price --rpc-tx-feecap --min-block-occupancy-ratio --miner-extra-data --permissions-nodes-config-file --permissions-accounts-config-file --permissions-nodes-contract-address --permissions-nodes-contract-version --permissions-accounts-contract-address --required-blocks --required-block --privacy-url --privacy-public-key-file --privacy-marker-transaction-signing-key-file --target-gas-limit --tx-pool-max-size --tx-pool-hashes-max-size --tx-pool-retention-hours --tx-pool-price-bump --key-value-storage --security-module --pruning-blocks-retained --pruning-block-confirmations --pid-path --api-gas-price-blocks --api-gas-price-percentile --api-gas-price-max --static-nodes-file --discovery-dns-url --banned-node-ids --banned-node-id --ethstats --ethstats-contact --node-private-key-file --logging -l --data-storage-format --bonsai-maximum-back-layers-to-load"
  local MODE_option_args="FULL FAST" # --sync-mode values
  local NETWORK_option_args="MAINNET RINKEBY ROPSTEN SEPOLIA GOERLI KILN DEV CLASSIC KOTTI MORDOR ECIP1049_DEV ASTOR" # --network values
  local natMethod_option_args="UPNP UPNPP2PONLY DOCKER KUBERNETES AUTO NONE" # --nat-method values
  local rpcHttpAuthenticationAlgorithm_option_args="RS256 RS384 RS512 ES256 ES384 HS256 ES512" # --rpc-http-authentication-jwt-algorithm values
  local rpcWebsocketsAuthenticationAlgorithm_option_args="RS256 RS384 RS512 ES256 ES384 HS256 ES512" # --rpc-ws-authentication-jwt-algorithm values
  local metricsProtocol_option_args="PROMETHEUS OPENTELEMETRY NONE" # --metrics-protocol values
  local dataStorageFormat_option_args="FOREST BONSAI" # --data-storage-format values
  ...
  case ${prev_word} in
    ...
    --rpc-http-api|--rpc-http-apis)
      return
      ;;
    --rpc-http-api-method-no-auth|--rpc-http-api-methods-no-auth)
      return
      ;;
    ...
    --rpc-ws-api|--rpc-ws-apis)
      return
      ;;
    --rpc-ws-api-methods-no-auth|--rpc-ws-api-method-no-auth)
      return
      ;;
    ...
    --logging|-l)
      ...
      ;;
    --data-storage-format)
      COMPREPLY=( $( compgen -W "${dataStorageFormat_option_args}" -- "${curr_word}" ) )
      return $?
      ;;
...
function _picocli_besu_publickey_export() {
  ...
  local commands=""
  local flag_opts="-h --help -V --version"
  local arg_opts="--node-private-key-file --ec-curve --to"
